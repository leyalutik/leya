...
//	}
//}
const std::string browser = "firefox.exe";
const std::string path_L = "open /home/leya_lutik/Desktop/leya/notion_set/";
const std::string path_W =  "start \"\"  firefox.exe file:///C:/git/leya/notion_set/";
...
void handle_option_f(const std::set<std::string>& notions, bool is_OS);
bool find_key(
	...
	while(option != 'e')
	{
		...
		switch(option){
			case 10	:	handle_option_Enter(notions);
					break;
			case 'f':	handle_option_f(notions, is_Windows);
					break;
			case 'e':	break;
...
void handle_option_f(const std::set<std::string>& notions, bool is_OS)
{
	...
	char correct = getchar();
	if(correct == 10 || correct == 'Y' || correct == 'y')
	{
		SYSTEM(found_key, is_OS);
